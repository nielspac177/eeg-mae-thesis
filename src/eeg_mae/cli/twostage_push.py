...
def main(argv: list[str] | None = None) -> None:
    ...
    p.add_argument("--cnn", default=None, help="timm CNN arch for a CNN two-stage model (e.g. efficientnet_b0)")
    p.add_argument("--lr-head", type=float, default=None, help="override head/base LR (CNNs want ~3e-4)")
    ...
    if args.cnn:
        ...
        name = args.name or f"twostage_cnn_{args.cnn}_v{args.min_votes_s1}-{args.min_votes_eval}"

        def model_factory():
            return make_effnet_classifier(args.cnn, n_classes=6).to(device)
    else:
    ...
    res = run_twostage_oof(cfg, model_factory, meta, load_fn=cache, device=device, progress=not args.no_progress)
    ...
    with out.open("a", newline="") as f:
        w = csv.DictWriter(f, fieldnames=["name", "backbone", "min_votes_s1", "min_votes_eval", "kl_overall"])
        if write_header:
            w.writeheader()
        w.writerow({"name": name, "backbone": args.cnn or f"mae_{args.pooling}",
                    "min_votes_s1": args.min_votes_s1,
